...
export const useWalletStore = create<WalletStore>((set, get) => ({
  ...
  connect: async () => {
    ...
    try {
      ...
      if (connectionResult.error) {
        throw new Error('Freighter wallet bulunamadı! Lütfen Freighter uzantısını yükleyin.');
      }

      if (!connectionResult.isConnected) {
        throw new Error('Freighter bağlı değil. Lütfen uzantıyı etkinleştirin.');
      }
      ...
      if (accessResult.error) {
        if (accessResult.error.includes('User declined')) {
          throw new Error('Kullanıcı wallet bağlantısını reddetti.');
        }
  ...
  switchNetwork: async (network: 'testnet' | 'mainnet') => {
    ...
    try {
      ...
      throw new Error(
        `Lütfen Freighter cüzdan ayarlarınızdan ${targetNetwork} ağına geçin, ardından tekrar bağlanın.`
      );

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Ağ değiştirme başarısız';
      set({ 
        isLoading: false, 
        error: errorMessage 
      });
